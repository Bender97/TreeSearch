...
int main(int argc, const char* argv[])
{
    ...
    //full = true;
    templName = "../dataset/03.jpg";
    Mat templ = loadImage(templName);
    imageName = "../Benchmark_step_1/Figure 3.jpg";
...
//    for (size_t i = 0; i < 10; ++i)
    {
        if (votes.at<uchar>(1, i) > 100 || votes.at<uchar>(1, i)<10) continue;
        Mat out = image.clone();
        Point2f pos(position[i][0], position[i][1]);
        float scale = position[i][2];
        float angle = position[i][3];

        cout << (int) votes.at<uchar>(1, i) << endl;
