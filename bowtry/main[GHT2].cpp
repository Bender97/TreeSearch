...
int main(int argc, const char* argv[])
{
    CommandLineParser cmd(argc, argv,
                          ...
                          "{ minScale       | 0.1       | minimal scale to detect }"
                          "{ maxScale       | 5         | maximal scale to detect }"
    ...
    //full = true;
    templName = "../dataset/02.jpg";
    Mat templ = loadImage(templName);
    //resize(templ, templ, Size(templ.cols/4, templ.rows/4));
    imageName = "../Benchmark_step_1/Figure2.jpg";
    ...
    Mat votes;
    TickMeter tm;
    namedWindow("cannied", WINDOW_AUTOSIZE);


    createTrackbar("canny low", "cannied", &canny_low, 10000, onChange, &templ);
    createTrackbar("canny high", "cannied", &canny_high, 10000, onChange, &templ);
    onChange(0, &templ);
    waitKey(0);
    return 0;

    int canny_threshold = 80;
        alg->setCannyLowThresh(canny_threshold);
        alg->setCannyHighThresh(3*canny_threshold);
        ...
        alg->detect(image, position, votes);
...
//    for (size_t i = 0; i < 10; ++i)
    {
        ...
        resize(out, out, Size(out.cols/4, out.rows/4));
        imshow("out", out);
        waitKey(0);
    }
